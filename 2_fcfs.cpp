...
{
    int id = -1, at = ct; // id stores p_id, at is made to give the process with least arrival time
    int i = p_size - 1;
    while (i >= 0)
    {
        if (!p[i].executed && p[i].arrival_time <= at)
        {
            at = p[i].arrival_time;
            id = i; // p_id of arrived and ready to execute process found till now
        }
        i--;
    }
